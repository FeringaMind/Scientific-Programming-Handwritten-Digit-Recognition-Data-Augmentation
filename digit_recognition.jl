...
function getdata(datadir)
    ...
    xtrain = reshape(xtrain, 28,28,1,:) |> Flux.flatten
...
md"""
## 2. Building a model
Next, we build a very simple neural network. This can be seen as a function of $f: \mathbb{R}^{784}\rightarrow \mathbb{R}^{10}$. The input to the function is the vectorized images. The output is a vector of numbers that indicate to which class the image belongs. The class with the largest number will be the class predicted by the network.
...
$$f(\mathbf{x}) = \sigma(\mathbf{W}\mathbf{x}+\mathbf{b})$$, where $\sigma$ is the softmax function. The function is parameterized by the weights $\mathbf{W}\in\mathbb{W}^{10\times784}$ and the biases $\mathbf{b}\in\mathbb{R}^{10}$.
"""

# ╔═╡ 4cad4ec3-08d4-4b57-ba49-4a9a63d89aff
function build_model(; imgsize=(28,28,1), nclasses=10)
    return Chain(
 	        Dense(prod(imgsize), nclasses),
            softmax)
end
